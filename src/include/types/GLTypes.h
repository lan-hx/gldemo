...
inline float dot(const QVector3D &v1, const QVector3D &v2) { return QVector3D::dotProduct(v1, v2); }
...
struct Plane {
 ...
 public:
  ...
  Plane(const QVector3D &normal, float distance) : normal_{normal.normalized()}, signed_distance_{distance} {}
  Plane(const QVector3D point, const QVector3D &normal)
      : normal_{normal.normalized()}, signed_distance_{-dot(point, this->normal_)} {}

  float getSignedDistanceToPoint(QVector3D point) const { return -dot(normal_, point) - signed_distance_; }
};
